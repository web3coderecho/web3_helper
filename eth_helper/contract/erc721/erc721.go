...
// Erc721InitializedIterator is returned from FilterInitialized and is used to iterate over the raw logs and unpacked data for Initialized events raised by the Erc721 contract.
type Erc721InitializedIterator struct {
	Event *Erc721Initialized // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *Erc721InitializedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(Erc721Initialized)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *Erc721InitializedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *Erc721InitializedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// Erc721Initialized represents a Initialized event raised by the Erc721 contract.
type Erc721Initialized struct {
	Version uint64
	Raw     types.Log // Blockchain specific contextual infos
}
...
func (_Erc721 *Erc721Filterer) FilterInitialized(opts *bind.FilterOpts) (*Erc721InitializedIterator, error) {

	logs, sub, err := _Erc721.contract.FilterLogs(opts, "Initialized")
	if err != nil {
		return nil, err
	}
	return &Erc721InitializedIterator{contract: _Erc721.contract, event: "Initialized", logs: logs, sub: sub}, nil
}

// WatchInitialized is a free log subscription operation binding the contract event 0xc7f505b2f371ae2175ee4913f4499e1f2633a7b5936321eed1cdaeb6115181d2.
//
// Solidity: event Initialized(uint64 version)
func (_Erc721 *Erc721Filterer) WatchInitialized(opts *bind.WatchOpts, sink chan<- *Erc721Initialized) (event.Subscription, error) {

	logs, sub, err := _Erc721.contract.WatchLogs(opts, "Initialized")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(Erc721Initialized)
				if err := _Erc721.contract.UnpackLog(event, "Initialized", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event Initialized(uint64 version)
func (_Erc721 *Erc721Filterer) ParseInitialized(log types.Log) (*Erc721Initialized, error) {
	event := new(Erc721Initialized)
	if err := _Erc721.contract.UnpackLog(event, "Initialized", log); err != nil {
		return nil, err
	}
